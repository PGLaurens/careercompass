      ...
      <div className="max-w-6xl mx-auto">
        ...
        <main className="grid lg:grid-cols-3 gap-8">
          <div className="lg:col-span-2 space-y-6">
            <Card className="shadow-lg">
              <CardHeader>
                ...
                <p className="text-sm text-primary mt-2 font-medium bg-primary/10 p-3 rounded-lg text-wrap">{results.primaryCareer.reasoning}</p>
              </CardHeader>
              <CardContent>
                <div className="space-y-6">
                  <div>
                    <h3 className="text-lg font-semibold text-gray-800 mb-4 flex items-center text-balance"><Clock className="w-5 h-5 mr-2 text-primary" />Your Journey Timeline</h3>
                    <div className="relative pl-6">
                        ...
                        <div key={index} className="flex items-start mb-4">
                            ...
                            <div className="pl-4">
                                <h4 className="font-semibold text-gray-800 text-balance">{stage.stage}</h4>
                                <p className="text-sm text-gray-600 text-wrap">{stage.duration}</p>
                                <p className="text-sm text-gray-700 mt-1 text-wrap">{stage.focus}</p>
                                <p className="text-xs text-gray-500 mt-1 text-wrap">{stage.details}</p>
                            </div>
                        </div>
                        ))}
                    </div>
                  ...
                  <div>
                    <h3 className="text-lg font-semibold text-gray-800 mb-3 flex items-center text-balance"><BarChart3 className="w-5 h-5 mr-2 text-primary" />What You'll Do Daily</h3>
                    <div className="grid md:grid-cols-2 gap-x-4 gap-y-2">
                      {results.primaryCareer.dailyTasks?.map((task, index) => (
                        <div key={index} className="flex items-center text-sm text-gray-700 text-wrap"><div className="w-1.5 h-1.5 bg-primary rounded-full mr-2 flex-shrink-0"></div>{task}</div>
                      ))}
                    </div>
                  </div>
                  <div>
                    ...
                    <Card key={index} className="shadow-lg">
                        ...
                        <CardContent>
                            <div className="flex items-center justify-between mb-2">
                                <h4 className="font-semibold text-gray-700 text-balance">{career.title}</h4>
                                <div className="text-lg font-bold text-primary">{career.matchPercentage}%</div>
                            </div>
                            <p className="text-sm text-gray-600 mb-3 text-wrap">{career.description}</p>
                            <div className="flex flex-wrap gap-1">
                                {career.subjects?.slice(0, 3).map((subject, i) => (
                                <span key={i} className="px-2 py-1 bg-gray-100 text-gray-700 rounded text-xs text-balance">{subject}</span>
                                ))}
                            </div>
                        </CardContent>
                    </Card>
          ...
          <aside className="space-y-6">
            <Card className="shadow-lg">
                <CardHeader><h3 className="text-lg font-semibold text-gray-800 flex items-center text-balance"><User className="w-5 h-5 mr-2 text-primary" />Personality Insights</h3></CardHeader>
                <CardContent className="space-y-4">
                    <div><h4 className="font-medium text-gray-700 text-balance">Personality Type</h4><p className="text-sm text-gray-600 mt-1 text-wrap">{results.insights.personalityType}</p></div>
                    <div><h4 className="font-medium text-gray-700 text-balance">Core Strengths</h4><ul className="text-sm text-gray-600 mt-1 list-disc list-inside text-wrap">{results.insights.strengths.map((s, i) => <li key={i}>{s}</li>)}</ul></div>
                    <div><h4 className="font-medium text-gray-700 text-balance">What Drives You</h4><ul className="text-sm text-gray-600 mt-1 list-disc list-inside text-wrap">{results.insights.motivations.map((m, i) => <li key={i}>{m}</li>)}</ul></div>
                    <div><h4 className="font-medium text-gray-700 text-balance">Work Style</h4><p className="text-sm text-gray-600 mt-1 text-wrap">{results.insights.workStyle}</p></div>
                </CardContent>
            </Card>
            <Card className="shadow-lg">
