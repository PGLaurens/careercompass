...
const CareerSchema = z.object({
    ...
    salary: z.string().describe('The expected *monthly* salary range for this career in the specified country. This should be generated by the getSalaryData tool.'),
...
const getSalaryData = ai.defineTool(
    {
      name: 'getSalaryData',
      description: 'Provides a localized *monthly* salary estimate for a given career title and country.',
    ...
    async ({ careerTitle, country }) => {
      ...
      return `${countryData.symbol}${lowMonthly.toLocaleString()} - ${countryData.symbol}${highMonthly.toLocaleString()} ${countryData.currency} / month`;
    }
);
...
const careerSuggester = ai.definePrompt({
    ...
    **Process:**
    ...
    4.  **Flesh out each career suggestion:** For EACH of the three careers, you must generate all fields in the CareerSchema. It is **critical** that all text is tailored to a 13-16 year old. This includes:
        ...
        *   A detailed \`timeline\` with at least 4-5 stages. For each stage that is a job (e.g., Junior, Mid-Level, Senior), you MUST include a localized *monthly* \`salary\` range. Use the overall salary from the \`getSalaryData\` tool as a baseline to estimate the progression for different experience levels.
    ...
    5.  **Use Tools for Localization (IMPORTANT):**
        *   For EACH of the three career suggestions, you MUST use the \`getSalaryData\` tool to get a localized *monthly* salary. This will serve as the baseline for your timeline salary estimates.
